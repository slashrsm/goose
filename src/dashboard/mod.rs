...
//!
//! Frontend is selected at **compile time** via Cargo features:
//! - `dashboard-js` (default) — vanilla JS + ECharts
//! - `dashboard-elm` — Elm 0.19 (`dashboard-elm/` + `scripts/build-dashboard-elm.sh`)
//! - `dashboard-wasm` — Yew WebAssembly (`dashboard-ui/` + `scripts/build-dashboard-ui.sh`)
//!
//! Precedence when several are enabled (e.g. defaults + `--features dashboard-elm`):
//! **wasm > elm > js**.

#[cfg(not(any(
    feature = "dashboard-js",
    feature = "dashboard-wasm",
    feature = "dashboard-elm"
)))]
compile_error!(
    "enable a dashboard frontend feature: `dashboard-js` (default), `dashboard-elm`, or `dashboard-wasm`"
);
...
// Precedence: wasm > elm > js (see module docs).
#[cfg(all(
    feature = "dashboard-js",
    not(feature = "dashboard-wasm"),
    not(feature = "dashboard-elm")
))]
const INDEX_HTML: &str = include_str!("static/index-js.html");
#[cfg(all(
    feature = "dashboard-js",
    not(feature = "dashboard-wasm"),
    not(feature = "dashboard-elm")
...
#[cfg(all(feature = "dashboard-elm", not(feature = "dashboard-wasm")))]
const INDEX_HTML: &str = include_str!("static/index-elm.html");
/// Generated by `scripts/build-dashboard-elm.sh` (`elm make` + SSE bootstrap).
#[cfg(all(feature = "dashboard-elm", not(feature = "dashboard-wasm")))]
const ELM_APP_JS: &str = include_str!("static/elm/app.js");

#[cfg(feature = "dashboard-wasm")]
const INDEX_HTML: &str = include_str!("static/index-wasm.html");
// Generated by `scripts/build-dashboard-ui.sh` (Yew → wasm32).
#[cfg(feature = "dashboard-wasm")]
const WASM_JS: &str = include_str!("static/wasm/goose_dashboard_ui.js");
#[cfg(feature = "dashboard-wasm")]
const WASM_BG: &[u8] = include_bytes!("static/wasm/goose_dashboard_ui_bg.wasm");
...
fn router(handle: DashboardHandle) -> Router {
    ...
    {
        app = app.route("/static/app.js", get(app_js));
    }

    #[cfg(all(feature = "dashboard-elm", not(feature = "dashboard-wasm")))]
    {
        app = app.route("/static/elm/app.js", get(elm_app_js));
    }

    #[cfg(feature = "dashboard-wasm")]
    {
        app = app
            .route("/static/wasm/goose_dashboard_ui.js", get(wasm_js))
            .route("/static/wasm/goose_dashboard_ui_bg.wasm", get(wasm_bg));
    }
