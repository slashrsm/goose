...
//!
//! If both features are enabled (common when adding `--features dashboard-wasm` without
//! `--no-default-features`), **WASM wins** so you do not need to disable defaults.

#[cfg(not(any(feature = "dashboard-js", feature = "dashboard-wasm")))]
compile_error!(
    "enable a dashboard frontend feature: `dashboard-js` (default) or `dashboard-wasm`"
);
...
// Prefer WASM when both features are on (see module docs).
#[cfg(all(feature = "dashboard-js", not(feature = "dashboard-wasm")))]
